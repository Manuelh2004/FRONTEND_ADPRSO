const TablaAdopcion = ({
  ...
  paginaActual,
  registrosPorPagina,
  confirmarVolverAPendiente, // Nuevo prop correcto
}) => {
  return (
          ...
          {adopcionesPaginados.map((adopcion, index) => {
            ...
            return (
              <tr key={adopcion.adop_id} className="border-t hover:bg-gray-100">
                ...
                <td className="px-6 py-4 text-center">
                  {adopcion.usuario.usr_nombre} {adopcion.usuario.usr_apellido}
                </td>
                <td className="px-6 py-4 text-center flex justify-center space-x-4">
                  ...
                  {(adopcion.adop_estado === 1 || adopcion.adop_estado === 2) && (
                    <button
                      className="text-blue-500 hover:text-blue-700 transition duration-300 cursor-pointer"
                      onClick={() => confirmarVolverAPendiente(adopcion)}
